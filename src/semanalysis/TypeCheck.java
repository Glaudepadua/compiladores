...
public class TypeCheck extends VarCheck {
	...
	protected final EntrySimple LIST_TYPE;
	...
	public TypeCheck() {
		...
		LIST_TYPE = (EntrySimple) Maintable.classFindUp("List");
		...
		types.put(DOUBLE_TYPE,
				new ArrayList<>(Arrays.asList(DOUBLE_TYPE, STRING_TYPE)));
		types.put(
				STRING_TYPE,
	...
	public void TypeCheckClassDeclNode(ClassDeclNode x)
		...
		if (circularSuperclass(nc, nc.parent)) { // se existe declaracão
													// circular, ERRO
			nc.parent = null;
		...
		for (p = x.vars; p != null; p = p.next) {
			...
			if (l != null) {
				throw new SemanticException(q.position, "Variable "
						+ q.position.image + " already declared");
			}
		}
	}
	...
	public void TypeCheckMethodBodyNode(MethodBodyNode x) {
		...
		TypeCheckLocalVarDeclListNode(x.param); // trata parâmetro como var.
												// local
	...
	public void TypeCheckSuperNode(SuperNode x) throws SemanticException {
		...
		// se não achou, ERRO
		if (m == null) {
			throw new SemanticException(x.position, "Constructor " + p.name
	...
	public type TypeCheckNewObjectNode(NewObjectNode x)
		...
		// se não achou, ERRO
		if (c == null) {
			throw new SemanticException(x.position, "Class " + x.name.image
					+ " not found");
		}
	...
	public type TypeCheckNewArrayNode(NewArrayNode x) throws SemanticException {
		...
		// se não achou, ERRO
		if (c == null) {
			throw new SemanticException(x.position, "Type " + x.name.image
					+ " not found");
		}
	...
	public type TypeCheckRelationalNode(RelationalNode x)
		...
		if ((op != ParserConstants.EQ) && (op != ParserConstants.NE)
				&& (t1.dim > 0)) {
	...
	public type TypeCheckUnaryNode(UnaryNode x) throws SemanticException {
		...
		// se dimensão > 0, ERRO
		if (t.dim > 0) {
			throw new SemanticException(x.position, "Can not use unary "
					+ x.position.image + " for arrays");
		}
	...
	public type TypeCheckIntConstNode(IntConstNode x) throws SemanticException {
		...
		} catch (NumberFormatException e) { // se deu erro, formato e inválido
	...
	public type TypeCheckCallNode(CallNode x) throws SemanticException {
		...
		if (!(t1.ty instanceof EntryClass)) {
			throw new SemanticException(x.position, "Type " + t1.ty.name
	...
	public type TypeCheckDotNode(DotNode x) throws SemanticException {
		...
		if (!(t.ty instanceof EntryClass)) {
			throw new SemanticException(x.position, "Type " + t.ty.name
		...
		// se não achou, ERRO
		if (v == null) {
			throw new SemanticException(x.position, "Variable " + x.field.image
					+ " not found in class " + c.name);
		}
